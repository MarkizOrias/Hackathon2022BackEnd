...
def test_transfer_ownership():
    ...
    print(
        f"Balance After All Clients Deployments Of Certs: {balance_after_deploying_certs}"
    )
    ...
    case_one = creator_contract.transOwnership(
        account_three,
        account_three,
        cert_to_move,
        {"from": account_three, "value": fee},
    )
    case_one.wait(1)
    ...
    print(
        f"Certificates Owned By Client 3 {account_three} Are: {get_certs_three_case_one}"
    )
    (
        all_certs_stored,
        certs_owned_by_client_two_case_one,
    ) = creator_contract.arrayLengthGetter(account_two, {"from": account_one})
    (
        all_certs_stored,
        certs_owned_by_client_three_case_one,
    ) = creator_contract.arrayLengthGetter(account_three, {"from": account_one})
    ...
    case_two = creator_contract.transOwnership(
        account_one, account_three, cert_to_move, {"from": account_one, "value": fee}
    )
    case_two.wait(1)
    ...
    print(
        f"Certificates Owned By Client 3 {account_three} Are: {get_certs_three_case_two}"
    )
    (
        all_certs_stored,
        certs_owned_by_client_two_case_two,
    ) = creator_contract.arrayLengthGetter(account_two, {"from": account_one})
    (
        all_certs_stored,
        certs_owned_by_client_three_case_two,
    ) = creator_contract.arrayLengthGetter(account_three, {"from": account_one})
    ...
    case_three = creator_contract.transOwnership(
        account_two, account_three, cert_to_move, {"from": account_two, "value": fee}
    )
    case_three.wait(1)
    ...
    print(
        f"Certificates Owned By Client 2 {account_two} Are: {get_certs_two_case_three}"
    )
    get_certs_three_case_three = creator_contract.getCertificatesYouOwn(account_three)
    print(
        f"Certificates Owned By Client 3 {account_three} Are: {get_certs_three_case_three}"
    )
    (
        all_certs_stored,
        certs_owned_by_client_two_case_three,
    ) = creator_contract.arrayLengthGetter(account_two, {"from": account_one})
    (
        all_certs_stored,
        certs_owned_by_client_three_case_three,
    ) = creator_contract.arrayLengthGetter(account_three, {"from": account_one})
    print(f"Amount Of Certs Owned By Client 2: {certs_owned_by_client_two_case_three}")
    print(
        f"Amount Of Certs Owned By Client 3: {certs_owned_by_client_three_case_three}"
    )
    all_certs_stored, certs_owned_by_client_one = creator_contract.arrayLengthGetter(
        account_one, {"from": account_one}
    )
    all_certs, certs_owned_by_clients = creator_contract.arrayLengthGetter(
        account_one, {"from": account_one}
    )
